...
describe("Migration flows: migrateconfig and migratesettings", function()
  ...
  it("migrateconfig moves disabled_plugins from prev_config to airplanemode and removes prev_config file", function()
    ...
    fh:write("old")
    fh:close()
    U:saveAPMsetting("disabled_plugins", { calibre = true, newsdownloader = true }, settings.prev_config)
    ...
    local moved = U:readAPMsetting(settings.koreader_plugins, settings.airplanemode)
    ...
    assert.is_false(package.loaded["modules/helpers"].isFile(settings.prev_config))
  end)
  ...
  it("migratesettings moves 'airplanemode' boolean and footer setting, and cleans old keys", function()
    ...
    U:saveAPMsetting("airplanemode", true, settings.koreader)
    U:saveAPMsetting("plugins_disabled", { a = true }, settings.airplanemode)
    U:saveAPMsetting("airplanemode_in_footer", true, settings.koreader)
    ...
    assert.is_true(U:APMisTrue("airplanemode_enabled", settings.airplanemode))
    -- old koreader key should be removed
    assert.is_false(U:APMhas("airplanemode", settings.koreader))
    -- footer moved
    assert.is_true(U:APMhas("airplanemode_in_footer", settings.airplanemode))
    assert.is_false(U:APMhas("airplanemode_in_footer", settings.koreader))
  end)
end)
