...
local PluginManager = package.loaded["modules/PluginManager"]
...
describe("AirPlaneMode actions - stop, delete, enable/disable", function()
  ...
  it("stopPlugin should restore plugin settings and toggle airplanemode off", function()
    ...
    package.loaded["modules/PluginManager"].restorePluginSettings = function()
      restored = true
    end
  ...
  it("deletePluginSettings should remove files and settings", function()
    ...
    U:saveAPMsetting("airplanemode", true, settings.airplanemode)
    U:saveAPMsetting("airplanemode_in_footer", true, settings.airplanemode)

    assert.is_true(package.loaded["modules/helpers"].isFile(settings.airplanemode))

    AP.deletePluginSettings()

    -- file should be removed
    assert.is_false(package.loaded["modules/helpers"].isFile(settings.airplanemode))
  ...
  it("Enable should set airplanemode and create a backup; Disable should clear it and remove backup", function()
    ...
    assert.is_false(package.loaded["modules/helpers"].isFile(settings.backup))
  end)
end)
