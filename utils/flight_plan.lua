...
function Updater.composeBranchUrl(branch)
  ...
  return string.format("https://api.github.com/repos/kodermike/airplanemode.koplugin/zipball/%s", encoded)
end
...
local function httpGetJSON(url, user_agent)
  ...
  local handle = io.popen(string.format("curl -s -L -H 'User-Agent: KOReader-AirPlaneMode' -H 'Accept: application/vnd.github.v3+json' %q", url))
  if handle then
...
function Updater.checkBackground(on_update_found)
  ...
  local now = os.time()
  if _last_check_time and (now - _last_check_time) < CHECK_INTERVAL then
    return
  end
  ...
  UIManager:scheduleIn(0.1, function()
    ...
    if isNewer(ver, settings.version) then
      _cached_version = ver
      _cached_zip_url = nil
  ...
  NetworkMgr:runWhenOnline(function()
    ...
    UIManager:scheduleIn(0.1, function()
      ...
      for _i, rel in ipairs(releases) do
        if rel.draft or rel.prerelease then
          goto continue
        end
      ...
      if #new_releases == 0 then
        UIManager:show(InfoMessage:new({
          text = _("AirPlaneMode is up to date.") .. "\n\n" .. _("Version: ") .. "v" .. settings.version,
          timeout = 3,
        }))
        return
      end
      ...
      local buttons = {
        {
          ...
          },
          {
            ...
            callback = function()
              ...
              Updater.install(latest_zip_url, settings.version, latest_version, on_success)
            end,
          },
        },
      }
      viewer = TextViewer:new({
        ...
        text = _("Installed: ") .. "v" .. settings.version .. "\n" .. _("Latest: ") .. "v" .. latest_version .. "\n\n" .. all_notes,
...
function Updater.install(zip_url, old_version, new_version, on_success, error_label)
  ...
  UIManager:scheduleIn(0.1, function()
    ...
    local cache_dir = DataStorage:getDataDir() .. "/data/airplanemode_cache"
    if lfs.attributes(cache_dir, "mode") ~= "directory" then
      lfs.mkdir(cache_dir)
    end
    local zip_path = cache_dir .. "/airplanemode.koplugin.zip"
        ...
        local ok_dl, code = pcall(function()
          ...
          local c = socket.skip(
            ...
            http.request({
              ...
              method = "GET",
              headers = {
                ["User-Agent"] = "KOReader-AirPlaneMode/" .. old_version,
              },
    ...
    UIManager:show(ConfirmBox:new({
      text = _("AirPlaneMode updated to v") .. new_version .. ".\n\n" .. _("Restart KOReader now?"),
      ok_text = _("Restart"),
      ok_callback = function()
...
function Updater.installBranch(branch, on_success)
  ...
  NetworkMgr:runWhenOnline(function()
    ...
    Updater.install(zip_url, settings.version, "branch:" .. branch, on_success, error_label)
  end)
end
...
function Updater.installLatestStable(on_success)
  ...
  NetworkMgr:runWhenOnline(function()
    ...
    UIManager:scheduleIn(0.1, function()
      ...
      Updater.install(zip_url, settings.version, new_version, on_success)
    end)
  end)
end
...
function Updater:resetToStableRelease()
  UIManager:show(ConfirmBox:new({
...
function Updater:editDevBranch(touchmenu_instance)
