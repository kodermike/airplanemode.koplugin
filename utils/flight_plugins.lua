--[[
FlightPlugins module for AirplaneMode
]]
---@class PluginEntry
---@field name string
...
---@return table
function FlightPlugins:default_plugins()
  return {
    ["archiveviewer"] = true,
    ["autodim"] = true,
    ["autostandby"] = true,
...
---@return PluginEntry[]
function FlightPlugins:getPlugins(builtin, settings)
  if settings.debug_is_on then
    local funcname = debug.getinfo(1, "n").name
    logger.dbg(
      funcname, "PluginManager - getPlugins - builtin: ", builtin, " settings: ", settings.koreader_plugins,
  ...
  local plugin_list = {}
  local BUILTIN_PLUGINS = self:default_plugins()
...
---@return nil
function FlightPlugins:disablePlugins(AirPlaneMode_Self, settings)
  --[[ start ]]
  if settings.debug_is_on then
    local funcname = debug.getinfo(1, "n").name
  ...
  if type(check_plugins) == "string" then
    ...
    for plugin, _ in pairs(check_plugins) do
      ...
      if disabled_plugins[plugin] ~= true then
        ...
        local modcheck = AirPlaneMode_Self.ui[plugin]
...
---@param settings table
---@return nil
function FlightPlugins:restorePluginSettings(settings)
...
---@param settings table
function FlightPlugins:enableCalibre(settings)
  -- re-set calibre_wirless to previous setting, or delete it if it didn't exist
  if U:FlightIsTrue("calibre_wireless", settings.backup) then
    if settings.debug_is_on then
      local funcname = debug.getinfo(1, "n").name
